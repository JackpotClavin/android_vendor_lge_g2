...
if [ \( -e "$DUMPREPORT.txt" \) -a \( -e "$SCREENSHOT" \) ]; then
    echo am broadcast -a com.lge.android.quickdump.intent.QUICKDUMP_COMPLETED --es path $READ_PATH_QUICKDUMP_READ
    am broadcast -a com.lge.android.quickdump.intent.QUICKDUMP_COMPLETED --es path $READ_PATH_QUICKDUMP_READ
else
    echo am broadcast -a com.lge.android.quickdump.intent.QUICKDUMP_FAILED
    am broadcast -a com.lge.android.quickdump.intent.QUICKDUMP_FAILED
fi
